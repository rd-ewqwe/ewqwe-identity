...
openssl req -new -x509 -days 3650 -key ewqwe.root.key.pem -out ewqwe.chain.pem \
    -subj "/CN=acme.com" \
    -addext "subjectAltName=DNS:ewqwe.local,DNS:localhost,IP:127.0.0.1" \
...
openssl x509 -req -sha256 -in server.csr -CA ewqwe.chain.pem -CAkey ewqwe.root.key.pem \
    ...
    -extfile <(cat server_extensions && echo "subjectAltName=DNS:ewqwe.local,DNS:localhost,IP:127.0.0.1") \
    -extensions v3_req
