...
# Clean up old files
rm -f ewqwe.root.key.pem ewqwe.ca.pem ewqwe.server.key.pem ewqwe.server.cert.pem
...
openssl req -new -x509 -days 3650 -key ewqwe.root.key.pem -out ewqwe.ca.pem \
    -subj "/CN=acme.com" \
...
openssl req -new -key ewqwe.server.key.pem -out server.csr \
    ...
    -addext "subjectAltName=DNS:demo.ewqwe.local,DNS:localhost,IP:127.0.0.1"
openssl x509 -req -sha256 -in server.csr -CA ewqwe.ca.pem -CAkey ewqwe.root.key.pem \
    -CAcreateserial -out ewqwe.server.cert.pem -days 3650 \
    -extfile <(cat server_extensions) \
    -extensions v3_req
...
openssl x509 -req  -sha256 -in user1.csr -CA ewqwe.ca.pem -CAkey ewqwe.root.key.pem \
    -CAcreateserial -out ewqwe.user1.cert.pem -days 3650 \
    -extfile <(cat user_extensions) \
    -extensions v3_req
...
openssl req -new -key ewqwe.user2.key.pem -out user2.csr \
    ...
    -addext "subjectAltName=DNS:localhost,IP:127.0.0.1"
openssl x509 -req  -sha256 -in user2.csr -CA ewqwe.ca.pem -CAkey ewqwe.root.key.pem \
...
cat ewqwe.server.cert.pem ewqwe.ca.pem > ewqwe.server.fullchain.pem
...
echo "  Root CA: ewqwe.ca.pem (key: ewqwe.root.key.pem)"
