...
5. **Cryptographic Signature Verification**:
   ...
   - **mDoc IssuerAuth COSE_Sign1**: The MSO is verified against the issuer certificate chain from the `x5chain` COSE header using OpenSSL. An untrusted or invalid chain is a hard error (`400 Bad Request`).
...
6. **Attestation Signing**: Generate an ES256-signed JWT attestation (see [Attestation JWT](#attestation-jwt)) confirming the verification outcome and embedding the verified credential claims
...
If `trusted_issuer_certs_dir` is omitted, the server looks for a directory named `issuer_certificates/` next to the configuration file. If that directory does not exist, no issuer CAs are trusted and all presentations will fail with `400 Bad Request`.

### Attestation Signing Certificate

The server signs attestation JWTs with an EC P-256 private key and includes its certificate in the JWKS endpoint so that Relying Parties can verify the signature independently.

```toml
# Path to the PEM certificate whose public key verifies the attestation JWT.
# The Subject CN of this certificate becomes the `iss` claim.
# Defaults to tls_params.server_certificate when omitted.
attestation_issuer_certificate = "certs/server.cert.pem"

# Path to the corresponding PEM private key (PKCS#8).
# Defaults to tls_params.server_private_key when omitted.
attestation_issuer_key = "certs/server.key.pem"
```

The **Subject CN** of the certificate is used as the `iss` claim in every attestation JWT, allowing RPs to identify the issuing verifier without additional configuration. When neither field is set, the server falls back to `tls_params.server_certificate` / `tls_params.server_private_key`.
...
# Attestation signing certificate and key.
# The Subject CN of the certificate becomes the `iss` claim in every attestation JWT.
# Defaults to tls_params.server_certificate / server_private_key when omitted.
# attestation_issuer_certificate = "certs/server.cert.pem"
# attestation_issuer_key         = "certs/server.key.pem"
...
Verifies a Verifiable Presentation (VP) token from a user's wallet. On success, returns a signed attestation JWT containing the verified claims. On failure, returns `success: false` with an attestation JWT that records the failed outcome (but no credential claims).
...
```json
{
  ...
  "attestation": "eyJhbGciOiJFUzI1NiIsImtpZCI6Ii10aEk1....<signed JWT>"
}
```
...
> **Important**: The `claims` field that appeared in earlier versions has been **removed**. All credential attribute claims are now embedded directly in the signed attestation JWT payload (see [Attestation JWT](#attestation-jwt)). RPs must verify the attestation signature before trusting any claims.

### `GET /api/openid4vp/.well-known/jwks.json` — Public JWKS

Returns a JWK Set (RFC 7517) that contains all public keys served by this instance. The JWKS has two logical groups:

| Key | Present when | Purpose |
| :-- | :----------- | :------ |
| JAR signing key | HAIP profile configured | Wallet verifies signed Authorization Requests (RFC 9101) |
| Attestation verification key | Always | RP verifies the `attestation` JWT from `POST /api/verify` |

The attestation key entry always includes:

- `kty`, `crv`, `x`, `y` — EC P-256 public key coordinates
- `alg: "ES256"` — signing algorithm
- `use: "sig"` — key usage
- `kid` — base64url-encoded SHA-256 fingerprint of the certificate DER (matches the `kid` header in every attestation JWT)
- `x5c` — the full certificate as a standard-base64 DER value (RFC 7517 §4.7)
...
Relying Parties should fetch this endpoint at startup (or on cache miss) to obtain the current attestation verification key.
...
Every call to `POST /api/verify` returns a signed **attestation JWT** — an ES256 compact-serialized JWT that cryptographically binds the verification outcome to a specific transaction. The RP must verify this signature before trusting any claim.
...
The `kid` value is the base64url-encoded SHA-256 fingerprint of the attestation signing certificate's DER representation. It is stable across restarts and matches exactly one key in the JWKS served at `/api/openid4vp/.well-known/jwks.json`.

### JWT Payload

| Claim | Type | Description |
| :---- | :--- | :---------- |
| `iss` | string | Subject CN of `attestation_issuer_certificate` (defaults to the TLS server cert CN) |
| `sub` | string | Transaction / attestation event ID (UUID) |
| `aud` | string | Relying party identifier (`client_id` from the verification request) |
| `exp` | number | Expiry Unix timestamp (5 minutes after issuance) |
| `iat` | number | Issued-at Unix timestamp |
| `nbf` | number | Not-before Unix timestamp |
| `jti` | string | Unique attestation UUID (replay prevention) |
| `verified` | boolean | `true` when the full cryptographic and policy pipeline succeeded |
| `nonce` | string? | Nonce from the original OpenID4VP request |
| `doc_type` | string? | Credential document type (e.g. `"org.iso.18013.5.1.mDL"`) |
| `namespace` | string? | Credential namespace (e.g. `"org.iso.18013.5.1"`) |
| _(credential claims)_ | mixed | Flattened verified attribute claims — only present when `verified: true` |

**Example payload** (mDL age verification, success):

```json
{
  "iss": "demo.ewqwe.local",
  "sub": "3f8a2e1c-0000-0000-0000-000000000001",
  "aud": "redirect_uri:https://rp.example.com",
  "exp": 1740820800,
  "iat": 1740820500,
  "nbf": 1740820500,
  "jti": "a1b2c3d4-...",
...
**Example payload** (failed verification — no credential claims):

```json
{
  "iss": "demo.ewqwe.local",
  "sub": "3f8a2e1c-0000-0000-0000-000000000002",
  "aud": "redirect_uri:https://rp.example.com",
  "exp": 1740820800,
  "iat": 1740820500,
  "nbf": 1740820500,
...
> **Security**: Credential claims (`age_over_18`, `family_name`, etc.) are **only populated when `verified: true`**. Never read claims from an attestation whose `verified` field is `false` or whose signature has not been verified.

### Verifying the Attestation (TypeScript)

The `@ewqwe/digital-identity` library provides a `parseAttestation()` helper that fetches the JWKS, locates the right key by `kid`, and performs signature verification in one call:

```typescript
import { parseAttestation } from "@ewqwe/digital-identity";

// Fetch the JWKS, find the key by kid, verify the ES256 signature, check exp/nbf.
const claims = await parseAttestation(
  result.attestation,           // compact JWT string from POST /api/verify
  "/api/openid4vp/.well-known/jwks.json",
);

console.log(claims.verified);       // true
console.log(claims.age_over_18);    // true (only present when verified === true)
console.log(claims.iss);            // "demo.ewqwe.local"
```

`parseAttestation` throws if:

- The JWKS cannot be fetched
- No key matching the JWT `kid` is found in the JWKS
- The ES256 signature is invalid
- The JWT is expired (`exp` in the past) or not yet valid (`nbf` in the future)

For manual / server-side verification use the lower-level `verifyAttestation(jwt, publicKey)` after importing the key with `importVerifierPublicKey(pemOrSpki)`.
...
| Attestation JWT | ES256 | `attestation_issuer_certificate` (or TLS server cert) | `jsonwebtoken` v9 | ✅ |
...
- `issuerAuth` COSE_Sign1 signature verified against the X.509 chain in the `x5chain` COSE header; an untrusted chain is a hard error (`400 Bad Request`)
...
The attestation signing key is loaded from disk on every request. For production deployments:

- Store the private key in a hardware security module (HSM) or secret manager (e.g. HashiCorp Vault, AWS Secrets Manager)
- Rotate the certificate periodically — the `kid` fingerprint in the JWKS and JWT header automatically tracks the current certificate
- Use a dedicated certificate for attestation signing distinct from the TLS server certificate (configure `attestation_issuer_certificate` and `attestation_issuer_key` explicitly)
...
- [x] ~~Attestation JWT signature verifiable via JWKS~~
- [x] ~~Credential claims only in signed attestation (no unauthenticated `claims` field)~~
- [ ] Replace test certificates with production certificates from trusted CA
- [ ] Use a dedicated `attestation_issuer_certificate` distinct from the TLS certificate
