...
x509_cert_path = "certs/server.fullchain.pem"   # leaf + intermediate + root, PEM
...
The provided `credential-server.toml` uses certificates generated by the `certificates/tls/` and `certificates/signer/` scripts:

```toml
host_name = "0.0.0.0"
host_port = 9443
default_username = "demo-user"
public_root_url = "https://id.demo.ewqwe.eu:4043"
...
The server records every verification event to a **hash-chained journal** for auditability. Each entry stores:
- Timestamp and authenticated username
- SHA-256 hash of the attestation JWT
- A chain hash linking the entry to the previous one (preventing retroactive tampering)
- Document type, namespace, and extracted credential claims
- Transaction `client_id`

Configure journal storage under `[journal_config]`:

```toml
[journal_config]
enabled = true
backend = "postgres"
url     = "postgres://user:pass@localhost/ewqwe_journal"
```

When journaling is enabled, all verification outcomes are persisted. The journal can be queried via:
- `GET /ewqwe_api/journal/{username}/entries` — paginated entries query
- `GET /ewqwe_api/journal/{username}/download` — CSV download

### Test Certificates

For development and testing, pre-generated certificates are available at the repository root:
- **TLS certificates**: `certificates/tls/`
- **Issuer CA certificates**: `certificates/issuers_cas/`
- **Signer certificates**: `certificates/signer/`

Generate new certificates using the provided scripts:
```bash
cd certificates/tls && ./generate_tls_certs.sh
cd certificates/signer && ./generate_signer_certs.sh
```
...
- **QR code generation**: Generates OpenID4VP authorization request QR codes
- **In-process verification**: VP tokens verified inline without HTTP round-trips
- **Audit journal**: Searchable, filterable journal with claims, doc type, and success/failure
- **Multi-language UI**: 9 languages (en, fr, de, it, es, sv, pl, cs, hr)
- **Customisable branding**: Company name, logo via admin settings
...
|-------|------|-------------|
| `POST /api/setup/bootstrap` | No | First-time admin creation |
| `POST /api/auth/login` | No | Password login |
| `POST /api/qr/generate` | Yes | Initiate QR transaction, sends `{credential_type, claims}` |
| `GET /api/qr/{id}/status` | Yes | Poll result: `{status, age_over_18, verified_claims}` |
| `GET /api/admin/journal` | Admin | Filterable audit journal |
| `PUT /api/admin/settings` | Admin | App settings |

### QR Flow

1. Verifier logs in → UI calls `POST /api/qr/generate` with `{credential_type, claims}`
2. QR code displayed → wallet scans & presents VP to `direct_post`
3. UI polls `GET /api/qr/{id}/status` every 2 seconds
4. On wallet response, server performs in-process verification
5. Poll returns `{status: "verified", age_over_18: bool, verified_claims: {...}}`
6. UI shows age result and list of verified claims (e.g. `age_over_18`, `portrait`)

### Default Claims

Default claims requested are `age_over_18` and `portrait`. The `portrait` claim is an mDoc base64-encoded image of the credential holder. Claims can be selected per-user in settings.
...
| Verifier | Email of the initiating user |
| Doc Type | e.g. `org.iso.18013.5.1.mDL` |
| Claims | e.g. `age_over_18, portrait` |
